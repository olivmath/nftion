...
describe("Token contract", function () {
	...
	describe("Transactions", function () {
		...
		it("`owner` must not send 100LUA to `addr1` because there is no balance", async function () {
			...
			expect(await luaToken.balanceOf(addr1.address)).to.equal(addr1Balance);
	...
	describe("Approve", function () {
		...
		it("`addr1` should allow `owner` spend 200LUA", async function () {
			await luaToken.connect(owner).transferFrom(addr1.address, addr2.address, 200)
		...
		it("`addr1` must not allow `owner` spend more than 200LUA", async function () {
			await expect(
				luaToken.connect(owner).transferFrom(addr1.address, addr2.address, 200 + 1)
			).to.be.revertedWith("ERC20: insufficient allowance");

			await expect(
				luaToken.connect(owner).transferFrom(addr1.address, addr2.address, 300)
			).to.be.revertedWith("ERC20: insufficient allowance");
		})
	})
	describe("Allowency", function () {
		...
		it("`owner` can spend 200LUA of `addr1`", async function () {
			const initAllowBalance = await luaToken.connect(addr1).allowance(addr1.address, owner.address)
		...
		it("`owner` can not spend any LUA of `addr1`", async function () {
			...
			await expect(
				luaToken.connect(owner).transferFrom(addr1.address, addr2.address, 200)
			).to.be.revertedWith("ERC20: insufficient allowance");
		})
	})
});